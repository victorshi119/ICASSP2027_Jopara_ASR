#!/usr/bin/env python3
"""New Table 1 row 9 (mixture of experts, code-switching slot empty): per-seed RAW scores (unrounded), mean +/- sample std over seeds 13/42/123,
and per-seed paired bootstrap against row 3 (auto-detect) and row 4a (Guarani mode)."""
import os
...
seeds = {13: "a2_nullcs_seed13_a2_test_hypotheses.jsonl", 42: "a2_nullcs_seed42_a2_test_hypotheses.jsonl", 123: "a2_nullcs_seed123_a2_test_hypotheses.jsonl"}
def cols(hy):
...
print("\nROW 9 (mean +/- sample std over 3 seeds), format CER / WER per column:")
cells = []
for j in range(5):
...
json.dump({"row9_cells": cells, "bootstrap": res}, open(Path(os.environ.get("JOPARA_OUT", Path(__file__).resolve().parent)) / "row9_seeds.json", "w"))
print("done")
